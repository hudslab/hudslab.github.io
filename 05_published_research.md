---
layout: post
title: "Published Research"
description: "Published journal articles of Land Use Lab"
image: /assets/images/paper_back.jpg
show_page_image: false
nav-menu: true
permalink: /published-research/
---

# Published Research / 게재 논문

Land Use Lab의 게재 논문 목록입니다.

---


### 2026
- **Multi-activity accessibility by public transit: Regional disparities in daily access to essential services**  
  *Applied Geography* — SSCI · IF 5.4 · Published

- **Exploring network scale separation strategies for car-bicycle integration**  
  *Transportation Research Interdisciplinary Perspectives* — ESCI · IF 3.8 · Published

### 2024
- **Deciphering long-term evolutionary patterns in inter-city travel networks in South Korea: Growth and decline of cities in association with inter-city connections**  
  *International Journal of Sustainable Transportation* — SSCI · IF 3.9 · Published

- **Changes in visitor behaviour across COVID-19 pandemic: Unveiling urban visitation dynamics and non-linear relationships with the built environment using mobile big data**  
  *Habitat International* — SSCI · IF 7.0 · Published

- **Preference-based jogging route selection in downtown Tokyo**  
  *Cities & Health* — SCOPUS · Published

- **Pedestrian movement with large-scale GPS records and transit-oriented development attributes**  
  *Sustainable Cities and Society* — SCIE · IF 12.0 · Published

- **Evaluating transit-oriented new town development: Insights from Seoul and Tokyo**  
  *Habitat International* — SSCI · IF 7.0 · Published

### 2023
- **Facility Resources Allocation Model Response to Spread of Infectious Diseases: Focusing on COVID-19 Isolation Facilities**  
  *국토계획* — KCI 우수등재지 · Published

### 2022
- **Cross-border evacuation and intermunicipal cooperation during large-scale flood disasters**  
  *International Journal of Disaster Risk Reduction* — SCIE · IF 4.5 · Published

### 2021
- **Measuring railway network performance considering accessibility levels in cities worldwide**  
  *Journal of Transport Geography* — SSCI · IF 6.3 · Published

- **Human Mobility Change Pattern and Influencing Factors during COVID-19, from the Outbreak to the Deceleration Stage: A Study of Seoul Metropolitan City**  
  *The Professional Geographer* — SSCI · IF 1.9 · Published

- **생활인프라 도보 거점 배치 모델 구축과 평가: 서울시 도봉구를 대상으로**  
  *국토계획* — KCI 우수등재지 · Published

- **Demographic, social, economic, and regional factors affecting the diffusion of hybrid electric vehicles in Japan**  
  *Energies* — SCIE · IF 3.2 · Published

- **How weather and special events affect pedestrian activities: volume, space, and time**  
  *International Journal of Sustainable Transportation* — SSCI · IF 3.9 · Published

- **Understanding Urban Flood Resilience in the Anthropocene: A Social–Ecological–Technological Systems (SETS) Learning Framework**  
  *Annals of the American Association of Geographers* — SSCI · IF 2.9 · Published

### 2019
- **A land-use mix allocation model considering adjacency, intensity, and proximity**  
  *International Journal of Geographical Information Science* — SCIE · IF 3.8 · Published

- **Spatial distribution of pedestrian space in central Tokyo**  
  *International Review for Spatial Planning and Sustainable Development* — ESCI · IF 1.7 · Published

